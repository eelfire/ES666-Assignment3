...
path = "Images{}*".format(
    os.sep
)  # Use os.sep, Windows, linux have different path delimiters
###
...
for idx, algo in enumerate(all_submissions):
    print(
        "****************\tRunning Awesome Stitcher developed by: {}  | {} of {}\t********************".format(
    ...
    #         stitched_image, homography_matrix_list = inst.make_panaroma_for_images_in(
    #             path=impaths
    #         )
    ...
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    PanoramaStitcher = getattr(module, "PanoramaStitcher")
    inst = PanoramaStitcher()
